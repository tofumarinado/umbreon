...
import { Mail, Phone, MapPin, ExternalLink, Briefcase, GraduationCap, Award, Code, Globe, Terminal, Sun, Moon } from "lucide-react";
    ...
    >
      ...
      <div className="pt-4 flex flex-wrap print:flex-col print:flex-nowrap gap-4 print:gap-1.5 items-center justify-center md:justify-start print:items-start text-slate-600 dark:text-slate-300 text-sm md:text-base">
        {resumeData.personal.contact.email && (
          <a href={`mailto:${resumeData.personal.contact.email}`} className="flex items-center gap-2 hover:text-slate-900 dark:hover:text-white transition-colors flat-card px-4 py-2 print:!p-0 print:!border-none print:!shadow-none print:!bg-transparent rounded-md flat-card-hover w-full md:w-auto">
        ...
        <a href={`https://${resumeData.personal.contact.linkedin}`} target="_blank" rel="noreferrer" className="flex items-center gap-2 hover:text-slate-900 dark:hover:text-white transition-colors flat-card px-4 py-2 print:!p-0 print:!border-none print:!shadow-none print:!bg-transparent rounded-md flat-card-hover w-full md:w-auto">
        ...
        <a href={`https://${resumeData.personal.contact.github}`} target="_blank" rel="noreferrer" className="flex items-center gap-2 hover:text-slate-900 dark:hover:text-white transition-colors flat-card px-4 py-2 print:!p-0 print:!border-none print:!shadow-none print:!bg-transparent rounded-md flat-card-hover w-full md:w-auto">
          <ExternalLink className="w-4 h-4 print:w-3 print:h-3 text-slate-500 dark:text-slate-400 shrink-0" /> <span className="print:text-[10px] print:tracking-tight">GitHub</span>
        </a>
      </div>
    </motion.header>
  );
...
const SkillsSection = () => {
  ...
  return (
    <motion.section
      variants={sectionVariants} initial="hidden" whileInView="visible" viewport={{ once: true, margin: "-100px" }}
    >
      <h2 className="text-2xl print:text-lg font-bold mb-8 print:mb-3 flex items-center gap-3 print:gap-0 border-b border-slate-300 dark:border-slate-700 pb-2 text-slate-900 dark:text-slate-100">
...
const ExperienceSection = () => {
  ...
  return (
    ...
    >
      <h2 className="text-2xl print:text-xl font-bold mb-10 print:mb-4 flex items-center gap-3 print:gap-0 border-b border-slate-300 dark:border-slate-700 pb-2 text-slate-900 dark:text-slate-100">
      ...
      <div className="space-y-8 print:space-y-3 relative before:absolute print:before:hidden before:inset-0 before:ml-4 before:-translate-x-px before:h-full before:w-1 before:bg-gradient-to-b before:from-brand-purple before:via-brand-pink before:to-transparent print:break-inside-auto print:block">
        {resumeData.experience.map((job: any, i: number) => (
          ...
          >
            {/* Timeline dot */}
            <div className="absolute print:hidden left-[0.35rem] top-8 w-6 h-6 -translate-y-1/2 rounded-sm border-4 border-slate-100 dark:border-slate-900 bg-brand-purple z-10 transition-transform duration-300 group-hover:scale-125 group-hover:bg-brand-cyan" />

            {/* Card */}
            <div className="w-full flat-card p-6 print:p-4 rounded-xl group-hover:-translate-y-1 transition-transform duration-300 print:border-slate-300">
              <div className="flex flex-col mb-3 print:mb-1.5">
                <span className="text-sm print:text-[10px] text-brand-blue dark:text-brand-cyan font-mono font-bold mb-1 print:mb-0.5">{job.startDate} - {job.endDate}</span>
                <h3 className="text-lg print:text-sm font-bold text-slate-900 dark:text-white uppercase tracking-wide print:leading-tight">{job.position}</h3>
                <h4 className="text-slate-600 dark:text-brand-pink font-semibold print:text-xs print:mt-0.5 print:leading-tight">{job.company}</h4>
                {job.location && <span className="text-xs text-slate-500 dark:text-slate-400 mt-1 print:mt-0 print:hidden flex items-center gap-1"><MapPin className="w-3 h-3" />{job.location}</span>}
              </div>
              {job.description && (
                <div className="text-sm print:text-[11px] text-slate-700 dark:text-slate-300 leading-relaxed mt-4 print:mt-1.5 space-y-1.5">
              ...
              {job.skills && (
                <div className="mt-4 flex flex-wrap gap-2 print:mt-1.5 print:gap-1">
                  {job.skills.map((s: any, idx: number) => (
                    <span key={idx} className="text-xs print:text-[8px] font-mono text-slate-600 dark:text-brand-cyan bg-slate-100 dark:bg-brand-cyan/10 px-2 py-0.5 rounded print:px-1 print:bg-transparent print:border print:border-slate-300">
                      {s.name}
                    </span>
                  ))}
                </div>
  ...
  return (
    ...
    >
      ...
      <div className="flat-card p-6 print:!p-0 print:!bg-transparent print:!border-none print:!shadow-none rounded-xl space-y-6 print:space-y-3">
        <div>
          <h3 className="text-xs uppercase tracking-widest text-slate-500 dark:text-slate-400 font-bold mb-3 print:mb-1 border-b border-slate-300 dark:border-slate-700 pb-2 print:pb-1">
          ...
          <ul className="space-y-2 print:space-y-1 mt-3 print:mt-1">
            {resumeData.certifications.map((cert: string, i: number) => (
              <li key={i} className="text-sm print:text-[11px] text-slate-700 dark:text-slate-200 flex items-start gap-2 font-medium print:break-inside-avoid">
                <span className="text-brand-blue dark:text-brand-cyan mt-1 print:mt-0">■</span> {cert}
              </li>
            ))}
          </ul>
        </div>
              ...
              <li key={i} className="text-sm flex items-start gap-2 print:break-inside-avoid-page">
                ...
                <div className="w-full">
                  <div className="flex justify-between items-start gap-2 mb-1.5">
                    ...
                    {award.year && <span className="text-slate-500 dark:text-brand-cyan text-xs font-mono font-bold mt-0.5">{award.year}</span>}
                  </div>
                  <div className="text-slate-600 dark:text-slate-400 text-xs leading-relaxed">{award.description}</div>
                </div>
              </li>
            ))}
          </ul>
...
export function ResumeCV() {
  ...
  return (
    <ResumeContext.Provider value={resumeData}>
      <div className="relative min-h-screen print:min-h-0 print:h-auto print:overflow-visible pb-24 text-slate-900 dark:text-slate-100 selection:bg-brand-pink selection:text-white transition-colors duration-300">

        {/* Global Controls */}
        <div className="fixed top-6 right-6 print:hidden z-50 flex items-center gap-3">
          <button
            onClick={() => setIsDark(!isDark)}
            className="flex items-center justify-center bg-white/80 dark:bg-slate-800/80 backdrop-blur-md border border-slate-200 dark:border-slate-700/50 hover:border-brand-purple/50 dark:hover:border-brand-purple/50 w-10 h-10 rounded-full hover:bg-slate-50 dark:hover:bg-slate-700 transition-all shadow-lg text-brand-purple"
            aria-label="Toggle dark mode"
          >
            {isDark ? <Moon className="w-5 h-5 text-brand-purple" /> : <Sun className="w-5 h-5 text-amber-500" />}
          </button>

          <button
            onClick={() => setLang(lang === 'EN' ? 'ES' : 'EN')}
            className="flex items-center gap-2 bg-white/80 dark:bg-slate-800/80 backdrop-blur-md border border-slate-200 dark:border-slate-700/50 hover:border-brand-cyan/50 dark:hover:border-brand-cyan/50 h-10 px-3 rounded-full hover:bg-slate-50 dark:hover:bg-slate-700 transition-all shadow-lg"
            aria-label="Toggle language"
          >
            <Globe className="w-5 h-5 text-brand-cyan" />
            <span className="text-xs font-bold uppercase w-5 text-center text-slate-700 dark:text-slate-200">{lang}</span>
          </button>
        </div>
